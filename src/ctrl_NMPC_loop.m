function [ctrl] = ctrl_NMPC(quad, pA, zA, yA, uA, aA) 
    %%
    import casadi.*
    ...
    REF = opti.parameter(7,1); % reference position [x,y,z,yaw]
...
%%
    opti.minimize(...
        pA*(1-REF(7))*( X(10,1:N) - REF(1) ) * ( X(10,1:N) - REF(1) )' + ...     % minimize X error
        pA*(1-REF(7))*( X(11,1:N) - REF(2) ) * ( X(11,1:N) - REF(2) )' + ...     % minimize Y error
        zA*(1-REF(7))*( X(12,1:N) - REF(3) ) * ( X(12,1:N) - REF(3) )' + ...     % minimize Z error
        yA *     ( X(6, 1:N) - REF(4) ) * ( X(6, 1:N) - REF(4) )' + ...     % minimize YAW error
        aA*REF(7) *( X(4, 1:N) - REF(5) ) * ( X(4, 1:N) - REF(5) )' + ...     % minimize PITCH error
        aA*REF(7) *( X(5, 1:N) - REF(6) ) * ( X(5, 1:N) - REF(6) )' + ...     % minimize ROLL error
        uA*(1-REF(7))*( sum(U,1) * sum(U,1)' ) )                                 % minimize total throttle use
